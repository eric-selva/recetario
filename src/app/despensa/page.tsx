...
interface CongeladorItem {
  ...
  servings: number
  added_at: string
  recipe?: { id: string; title: string; image_url: string | null; meal_type: string }
}
...
export default function DespensaPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8">
      ...
      <p className="mt-1 text-sm text-muted">Controla lo que tienes en despensa y congelador</p>
...
function NeveraTab() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      fetch(`/api/ingredientes?search=${encodeURIComponent(search)}`)
        .then((r) => r.json())
    ...
    <>
        ...
        {showSuggestions && search.trim().length > 0 && (
          <div className="absolute z-20 mt-1 max-h-60 w-full overflow-y-auto rounded-xl border border-border bg-card shadow-lg">
            ...
            >
              ...
              <span>Añadir &quot;<span className="font-semibold">{search.trim()}</span>&quot;</span>
            </button>
          </div>
        )}
      </div>
      ...
      {loading ? (
        ...
        <div className="mt-16 flex flex-col items-center gap-4 text-center">
          ...
          <div>
            <p className="font-heading text-lg font-semibold">La despensa esta vacia</p>
            <p className="mt-1 text-sm text-muted">Busca ingredientes para añadirlos.</p>
          </div>
        </div>
      ) : (
        <>
          <div className="mt-2 flex items-center justify-between">
            <p className="text-sm text-muted">{items.length} ingrediente{items.length !== 1 ? 's' : ''}</p>
            <button
              onClick={clearAll}
          ...
          <ul className="mt-3 space-y-2">
            {items.map((item) => (
              <li key={item.id} className="flex items-center gap-3 rounded-xl border border-border bg-card px-4 py-3 shadow-sm">
...
function CongeladorTab() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      fetch(`/api/recetas?search=${encodeURIComponent(search)}`)
        .then((r) => r.json())
        .then((data) => setSuggestions(Array.isArray(data) ? data : []))
    }, 200)
    return () => clearTimeout(timer)
  ...
  async function addRecipe(recipeId: string) {
    ...
    await fetch('/api/despensa', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ location: 'congelador', recipe_id: recipeId, servings: 4 }),
    })
    fetchItems()
  }
    ...
    <>
          ...
          <div className="absolute z-20 mt-1 max-h-60 w-full overflow-y-auto rounded-xl border border-border bg-card shadow-lg">
            {suggestions.map((recipe) => (
              ...
              >
                ...
                <span className={`shrink-0 rounded px-1.5 py-0.5 text-[10px] font-medium ${mealTypeStyles[recipe.meal_type] || ''}`}>
      ...
      {loading ? (
        ...
        <div className="mt-16 flex flex-col items-center gap-4 text-center">
          ...
          <div>
            <p className="font-heading text-lg font-semibold">El congelador esta vacio</p>
            <p className="mt-1 text-sm text-muted">Busca recetas para añadirlas.</p>
          </div>
        </div>
      ) : (
        <>
          <div className="mt-2 flex items-center justify-between">
            <p className="text-sm text-muted">{items.length} receta{items.length !== 1 ? 's' : ''}</p>
            <button
              onClick={clearAll}
              className="text-xs font-medium text-red-500 hover:text-red-600"
            >
              ...
              <li key={item.id} className="flex items-center gap-3 rounded-xl border border-border bg-card px-4 py-3 shadow-sm">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    {item.recipe && (
                      <span className={`shrink-0 rounded px-1.5 py-0.5 text-[10px] font-medium ${mealTypeStyles[item.recipe.meal_type] || ''}`}>
                        {mealTypeLabels[item.recipe.meal_type] || ''}
                      </span>
                    )}
                    <p className="truncate text-sm font-medium">{item.recipe?.title ?? 'Receta'}</p>
                  </div>
                </div>
                ...
                <div className="flex shrink-0 items-center gap-1.5">
                  ...
                  <span className="w-5 text-center text-sm font-semibold">{item.servings}</span>
                  <button
                    onClick={() => updateServings(item.id, Math.min(8, item.servings + 1))}
